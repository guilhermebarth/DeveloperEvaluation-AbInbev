...
using Ambev.DeveloperEvaluation.WebApi.Features.Carts.GetListCart;
...
namespace Ambev.DeveloperEvaluation.WebApi.Features.Carts
{
    ...
    public class CartsController : BaseController
    {
        ...
        public async Task<IActionResult> UpdateCart([FromRoute] int id, [FromBody] UpdateCartRequest request, CancellationToken cancellationToken)
        {
            ...
            var command = _mapper.Map<UpdateCartCommand>(request.Id);
            await _mediator.Send(command, cancellationToken);
        ...
        [HttpGet("")]
        [ProducesResponseType(typeof(ApiResponseWithData<GetListCartResponse>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status404NotFound)]
        public async Task<IActionResult> GetListCart(CancellationToken cancellationToken)
        {
            var request = new GetListCartRequest();
            var validator = new GetListCartRequestValidator();
            var validationResult = await validator.ValidateAsync(request, cancellationToken);

            if (!validationResult.IsValid)
                return BadRequest(validationResult.Errors);

            var command = _mapper.Map<GetListCartCommand>(request);
            var response = await _mediator.Send(command, cancellationToken);
